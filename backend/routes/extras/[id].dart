...
Future<Response> onRequest(RequestContext context, String id) async {
  switch (context.request.method) {
    case HttpMethod.put:
      return _updateExtra(context, id);
    case HttpMethod.patch:
      return _patchExtra(context, id);
    default:
      return Response(statusCode: HttpStatus.methodNotAllowed);
  }
}

Future<Response> _updateExtra(RequestContext context, String id) async {
  ...
  try {
    ...
    final duplicate = await conn.query(
      'SELECT id FROM public.extras WHERE LOWER(nombre) = LOWER(@nombre) AND id != @id',
      substitutionValues: {'nombre': nombre, 'id': id},
    );
    if (duplicate.isNotEmpty) {
      return Response.json(statusCode: 409, body: {'error': 'Ya existe otro extra con ese nombre.'});
    }
    ...
    if (update.isEmpty) {
      return Response.json(statusCode: 404, body: {'error': 'Extra no encontrado'});
    }
...
Future<Response> _patchExtra(RequestContext context, String id) async {
  final payload = AuthHelper.verifyToken(context);
  if (payload == null) {
    return Response.json(statusCode: 401, body: {'error': 'No autorizado'});
  }
