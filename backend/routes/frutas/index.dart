...
Future<Response> onRequest(RequestContext context) async {
  switch (context.request.method) {
    case HttpMethod.get:
      return _getFrutas(context);
    case HttpMethod.post:
      return _createFruta(context);
    default:
      return Response(statusCode: HttpStatus.methodNotAllowed);
  }
}

Future<Response> _getFrutas(RequestContext context) async {
  ...
  try {
    ...
    final query = soloDisponibles
        ? 'SELECT id, nombre, precio_adicional, disponible, imagen_url, created_at FROM public.frutas WHERE disponible = true ORDER BY nombre ASC'
        : 'SELECT id, nombre, precio_adicional, disponible, imagen_url, created_at FROM public.frutas ORDER BY nombre ASC';
...
Future<Response> _createFruta(RequestContext context) async {
  ...
  try {
    ...
    final duplicate = await conn.query(
      'SELECT id FROM public.frutas WHERE LOWER(nombre) = LOWER(@nombre)',
      substitutionValues: {'nombre': nombre},
    );
    if (duplicate.isNotEmpty) {
      return Response.json(statusCode: 409, body: {'error': 'Ya existe una fruta con ese nombre.'});
    }

    final insert = await conn.query('''
      INSERT INTO public.frutas (nombre, precio_adicional, disponible, imagen_url)
